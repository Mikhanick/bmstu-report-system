...
Сборка pdf-synctex нужна для использования совместно с техворксом/техстудио, чтобы синхронизировать строки между редактором и пдфкой.
pdf-synctex поддерживается только пдфлатехом от MikTeX, у техлайва нет флага для компиляции исходников из другой папки.

Чтобы проинициализировать папку для отчета необходимо запустить скрипт init_script.sh из папки, куда необходимо вставить папку report, либо непосредственно из папки report. В src будут положены шаблоны для отчета, в scripts -- скрипты автоматизации. при наличии файлов в src заменены они не будут.
...
При использовании с системой, таблица вставится автоматически при
```latex
\input{tables/table}
```
...
# 📋 Листинги кода (краткая инструкция)

## Разметка кода

В исходниках добавьте маркеры в комментариях:

```cpp
// #begin_listing <тег> <название>
...код...
// #end_listing
```
...
| Команда | Описание |
|---------|----------|
| `make SOURCE_DIR=../code init` | Создать промежуточные файлы |
| `make SOURCE_DIR=../code listings` | Обновить и сгенерировать .tex |
| `make SOURCE_DIR=../code update-all` | Принудительно обновить всё |
| `make clean` | Удалить .tex файлы |
| `make distclean` | Удалить всё |

## Подключение в LaTeX

```latex
\input{build/listings/<тег>.tex}
```

## Настройки (в первой строке `src/listings/<тег>@@<путь>.txt`)

```
45 90 --keep-comments --no-translit @@ Название
```

| Параметр | Описание |
|----------|----------|
| `45 90` | Разрывы после указанных строк |
| `--keep-comments` | Сохранить комментарии |
| `--no-translit` | Не транслитерировать |

## Автоматическое обновление

`make listings` сам проверяет изменения исходников и обновляет только изменённые файлы.
...
- Транслит: **включён**
- Нумерация частей: **ч.1, ч.2...** (если >1 части)