#!/bin/bash
set -e

# Определение расположения скрипта
script_dir=$(dirname "$(realpath "$0")")
src_dir="$script_dir/report"

# Определение целевой директории
current_dir=$(pwd)
if [ "$(basename "$current_dir")" = "report" ]; then
    dest_dir="$current_dir"
    operation_mode="обновление текущей папки report"
else
    dest_dir="$current_dir/report"
    operation_mode="копирование в новую папку report"
fi

# Цвета для вывода
GREEN='\033[1;32m'
YELLOW='\033[1;33m'
RED='\033[1;31m'
BLUE='\033[1;34m'
NC='\033[0m' # No Color

# Проверка существования исходной директории
if [ ! -d "$src_dir" ]; then
    echo -e "${RED}❌ Ошибка: Исходная папка не найдена: $src_dir${NC}" >&2
    echo -e "${YELLOW}ℹ️  Скрипт должен находиться в той же директории, что и папка report${NC}"
    exit 1
fi

# Проверка обязательных компонентов
for item in makefile scripts src; do
    if [ ! -e "$src_dir/$item" ]; then
        echo -e "${RED}❌ Ошибка: Отсутствует обязательный элемент: $src_dir/$item${NC}" >&2
        exit 1
    fi
done

echo -e "${BLUE}📋 Начало операции: $operation_mode${NC}"
echo -e "${BLUE}📦 Источник: ${src_dir}${NC}"
echo -e "${BLUE}📁 Назначение: ${dest_dir}${NC}"
echo -e "${BLUE}────────────────────────────────────────${NC}"

# Создание целевой директории (если нужно)
if [ ! -d "$dest_dir" ]; then
    mkdir -p "$dest_dir"
    echo -e "${YELLOW}🆕 Создана целевая директория: $dest_dir${NC}"
fi

# Копирование makefile с полной перезаписью
echo -e "${BLUE}📄 Копирование makefile с перезаписью...${NC}"
cp -vf "$src_dir/makefile" "$dest_dir/"

# Копирование scripts с полной перезаписью (рекурсивно)
echo -e "${BLUE}🔄 Копирование scripts с полной перезаписью...${NC}"
mkdir -p "$dest_dir/scripts"
cp -r --update=all -vf "$src_dir/scripts/"* "$dest_dir/scripts/" 2>/dev/null || true

# Копирование src только для новых файлов (--update=none)
echo -e "${BLUE}🆕 Копирование src (только новые файлы, без перезаписи)...${NC}"
mkdir -p "$dest_dir/src"
cp -r --update=none -vf "$src_dir/src/"* "$dest_dir/src/" 2>/dev/null || true

echo -e "\n${GREEN}✅ Успешно завершено!${NC}"
echo -e "• makefile и scripts: ${YELLOW}перезаписаны${NC}"
echo -e "• src: ${GREEN}добавлены только новые файлы${NC}"
echo -e "• Результат: ${dest_dir}"